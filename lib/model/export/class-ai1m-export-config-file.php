...
class Ai1m_Export_Config_File {
	...
	public static function execute( $params ) {
		...
		if ( isset( $params['archive_bytes_offset'] ) ) {
			...
			$archive_bytes_offset = ai1m_archive_bytes( $params );
		}
		...
		if ( isset( $params['total_package_size'] ) ) {
			...
			$total_package_size = ai1m_package_bytes( $params );
		}
		...
		// Set progress
		Ai1m_Status::info( sprintf( __( 'Archiving configuration file...<br />%d%% complete', AI1M_PLUGIN_NAME ), $progress ) );
		...
		if ( $archive->add_file( ai1m_package_path( $params ), AI1WM_PACKAGE_NAME, $package_bytes_written, $package_bytes_offset ) ) {

			// Set progress
			Ai1m_Status::info( __( 'Done archiving configuration file.', AI1M_PLUGIN_NAME ) );
