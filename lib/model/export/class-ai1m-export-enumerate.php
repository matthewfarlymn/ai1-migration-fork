...
class Ai1m_Export_Enumerate {
	...
	public static function execute( $params ) {
		...
		// Set progress
		Ai1m_Status::info( __( 'Retrieving a list of all WordPress files...', AI1M_PLUGIN_NAME ) );

		// Set exclude filters
		$exclude_filters = ai1m_content_filters();
		...
		if ( isset( $params['options']['no_plugins'] ) ) {
			...
			// Set exclude filters
			$exclude_filters = array_merge( $exclude_filters, ai1m_plugin_filters( $inactive_plugins ) );
		}
		...
		// Create map file
		$filemap = ai1m_open( ai1m_filemap_path( $params ), 'w' );

		// Iterate over content directory
		$iterator = new Ai1m_Recursive_Directory_Iterator( WP_CONTENT_DIR );

		// Exclude new line file names
		$iterator = new Ai1m_Recursive_Newline_Filter( $iterator );

		// Exclude uploads, plugins or themes
		$iterator = new Ai1m_Recursive_Exclude_Filter( $iterator, apply_filters( 'ai1m_exclude_content_from_export', $exclude_filters ) );
		...
		foreach ( $iterator as $item ) {
			if ( $item->isFile() ) {
				if ( ai1m_write( $filemap, $iterator->getSubPathName() . PHP_EOL ) ) {
					$total_files_count++;
		...
		// Set progress
		Ai1m_Status::info( __( 'Done retrieving a list of all WordPress files.', AI1M_PLUGIN_NAME ) );
