...
class Ai1m_Export_Database {
	...
	public static function execute( $params ) {
		...
		// Set progress
		Ai1m_Status::info( sprintf( __( 'Exporting database...<br />%d%% complete', AI1M_PLUGIN_NAME ), $progress ) );
		...
		if ( isset( $params['options']['no_spam_comments'] ) ) {
			$mysql->set_table_where_clauses( ai1m_table_prefix() . 'comments', array( "`comment_approved` != 'spam'" ) )
				->set_table_where_clauses( ai1m_table_prefix() . 'commentmeta', array( sprintf( "`comment_ID` IN ( SELECT `comment_ID` FROM `%s` WHERE `comment_approved` != 'spam' )", ai1m_table_prefix() . 'comments' ) ) );
		}

		// Exclude post revisions
		if ( isset( $params['options']['no_post_revisions'] ) ) {
			$mysql->set_table_where_clauses( ai1m_table_prefix() . 'posts', array( "`post_type` != 'revision'" ) );
		}
		...
		// Set table prefixes
		if ( ai1m_table_prefix() ) {
			$old_table_prefixes[] = $old_column_prefixes[] = ai1m_table_prefix();
			$new_table_prefixes[] = $new_column_prefixes[] = ai1m_servmask_prefix();
		} else {
			// Set table prefixes based on table name
			foreach ( $mysql->get_tables() as $table_name ) {
				$old_table_prefixes[] = $table_name;
				$new_table_prefixes[] = ai1m_servmask_prefix() . $table_name;
			}

			// Set table prefixes based on column name
			foreach ( array( 'user_roles' ) as $option_name ) {
				$old_column_prefixes[] = $option_name;
				$new_column_prefixes[] = ai1m_servmask_prefix() . $option_name;
			}
			...
			foreach ( array( 'capabilities', 'user_level', 'dashboard_quick_press_last_post_id', 'user-settings', 'user-settings-time' ) as $meta_key ) {
				...
				$new_column_prefixes[] = ai1m_servmask_prefix() . $meta_key;
			}
		}
		...
		if ( ai1m_table_prefix() ) {
			$include_table_prefixes[] = ai1m_table_prefix();
		} else {
			foreach ( $mysql->get_tables() as $table_name ) {
		...
		$mysql->set_table_where_clauses( ai1m_table_prefix() . 'options', array( sprintf( "`option_name` NOT IN ('%s', '%s', '%s', '%s', '%s', '%s', '%s')", AI1WM_ACTIVE_PLUGINS, AI1WM_ACTIVE_TEMPLATE, AI1WM_ACTIVE_STYLESHEET, AI1WM_STATUS, AI1WM_SECRET_KEY, AI1WM_AUTH_USER, AI1WM_AUTH_PASSWORD ) ) );

		// Replace table prefix on columns
		$mysql->set_table_prefix_columns( ai1m_table_prefix() . 'options', array( 'option_name' ) )
			->set_table_prefix_columns( ai1m_table_prefix() . 'usermeta', array( 'meta_key' ) );

		// Export database
		if ( $mysql->export( ai1m_database_path( $params ), $table_index, $table_offset ) ) {

			// Set progress
			Ai1m_Status::info( __( 'Done exporting database.', AI1M_PLUGIN_NAME ) );
