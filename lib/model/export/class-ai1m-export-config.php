...
class Ai1m_Export_Config {

	public static function execute( $params ) {
		global $wp_version, $wpdb;

		// Set progress
		Ai1m_Status::info( __( 'Preparing configuration file...', AI1M_PLUGIN_NAME ) );
		...
		// Set active plugins
		$config['Plugins'] = array_values( array_diff( ai1m_active_plugins(), ai1m_active_servmask_plugins() ) );

		// Set active template
		$config['Template'] = ai1m_active_template();

		// Set active stylesheet
		$config['Stylesheet'] = ai1m_active_stylesheet();

		// Save package.json file
		$handle = ai1m_open( ai1m_package_path( $params ), 'w' );
		ai1m_write( $handle, json_encode( $config ) );
		ai1m_close( $handle );

		// Set progress
		Ai1m_Status::info( __( 'Done preparing configuration file.', AI1M_PLUGIN_NAME ) );
