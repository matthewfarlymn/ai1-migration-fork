...
class Ai1m_Updater {
	...
	public static function check_for_updates() {
		...
		// Get extension updates
		foreach ( Ai1m_Extensions::get() as $slug => $extension ) {
	...
	public static function plugin_row_meta( $links, $file ) {
		...
		foreach ( Ai1m_Extensions::get() as $slug => $extension ) {
			...
			if ( $file === $extension['basename'] ) {
				...
				// Get updater URL
				$updater_url = add_query_arg( array( 'ai1m_updater' => 1 ), network_admin_url( 'plugins.php' ) );

				// Check Purchase ID
				if ( get_option( $extension['key'] ) ) {

					// Add "Check for updates" link
					$links[] = Ai1m_Template::get_content( 'updater/check', array(
						'url' => $updater_url,
					) );
					...
					// Add modal
					$links[] = Ai1m_Template::get_content( 'updater/modal', array(
						'url'   => $updater_url,
						'modal' => $modal_index,
					) );
