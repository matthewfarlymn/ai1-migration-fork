...
class Ai1m_Import_Blogs {

	public static function execute( $params ) {

		// Set progress
		Ai1m_Status::info( __( 'Preparing blogs...', AI1M_PLUGIN_NAME ) );
				...
				if ( isset( $multisite['Sites'] ) && ( $sites = $multisite['Sites'] ) ) {
					if ( count( $sites ) === 1 && ( $subsite = current( $sites ) ) ) {
						...
						throw new Ai1m_Import_Exception(
							__( 'The archive should contain <strong>Single WordPress</strong> site! Please revisit your export settings.', AI1M_PLUGIN_NAME )
						);
					}
				} else {
					throw new Ai1m_Import_Exception(
						__( 'At least <strong>one WordPress</strong> site should be presented in the archive.', AI1M_PLUGIN_NAME )
					);
				}
			} else {
				throw new Ai1m_Import_Exception(
					__( 'Unable to import <strong>WordPress Network</strong> into WordPress <strong>Single</strong> site.', AI1M_PLUGIN_NAME )
				);
			}
		}

		// Write blogs.json file
		$handle = ai1m_open( ai1m_blogs_path( $params ), 'w' );
		ai1m_write( $handle, json_encode( $blogs ) );
		ai1m_close( $handle );

		// Set progress
		Ai1m_Status::info( __( 'Done preparing blogs.', AI1M_PLUGIN_NAME ) );
