	...
	public static function execute( $params ) {
		...
		if ( isset( $package['PHP']['Version'] ) ) {
			if ( version_compare( $package['PHP']['Version'], '7.0.0', '<' ) && version_compare( PHP_VERSION, '7.0.0', '>=' ) ) {
				$messages[] = __(
					'<i class="ai1m-import-info">Your backup is from a PHP 5 but the site that you are importing to is PHP 7. ' .
		...
		// Set progress
		Ai1m_Status::confirm( implode( $messages ) );
		exit;
	}
}
