...
class Ai1m_Import_Content {
	...
	public static function execute( $params ) {
		...
		// Set progress
		Ai1m_Status::info( sprintf( __( 'Restoring %d files...<br />%d%% complete', AI1M_PLUGIN_NAME ), $total_files_count, $progress ) );
		...
		foreach ( $blogs as $blog ) {
			if ( ai1m_main_site( $blog['Old']['BlogID'] ) === false ) {
				if ( defined( 'UPLOADBLOGSDIR' ) ) {
					// Old sites dir style
					$old_paths[] = ai1m_files_path( $blog['Old']['BlogID'] );
					$new_paths[] = ai1m_files_path( $blog['New']['BlogID'] );

					// New sites dir style
					$old_paths[] = ai1m_sites_path( $blog['Old']['BlogID'] );
					$new_paths[] = ai1m_files_path( $blog['New']['BlogID'] );
				} else {
					// Old sites dir style
					$old_paths[] = ai1m_files_path( $blog['Old']['BlogID'] );
					$new_paths[] = ai1m_sites_path( $blog['New']['BlogID'] );
