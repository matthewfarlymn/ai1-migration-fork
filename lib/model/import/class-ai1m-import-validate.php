...
class Ai1m_Import_Validate {

	public static function execute( $params ) {

		// Verify file if size > 2GB and PHP = 32-bit
		if ( ! ai1m_is_filesize_supported( ai1m_archive_path( $params ) ) ) {
			throw new Ai1m_Import_Exception(
				__(
		...
		if ( isset( $params['total_archive_size'] ) ) {
			...
			$total_archive_size = ai1m_archive_bytes( $params );
		}
		...
		// Set progress
		Ai1m_Status::info( sprintf( __( 'Unpacking archive...<br />%d%% complete', AI1M_PLUGIN_NAME ), $progress ) );

		// Open the archive file for reading
		$archive = new Ai1m_Extractor( ai1m_archive_path( $params ) );
		...
		$allowed_size = apply_filters( 'ai1m_max_file_size', AI1WM_MAX_FILE_SIZE );
		...
		if ( $archive->has_not_reached_eof() ) {
			...
			if ( ( $completed = $archive->extract_by_files_array( ai1m_storage_path( $params ), array( AI1WM_PACKAGE_NAME, AI1WM_MULTISITE_NAME, AI1WM_DATABASE_NAME ), array(), $file_bytes_written, $file_bytes_offset ) ) ) {
				$file_bytes_offset = 0;
			}
		...
		if ( $archive->has_reached_eof() ) {
			...
			if ( false === is_file( ai1m_package_path( $params ) ) ) {
				throw new Ai1m_Import_Exception(
					__(
						'Please make sure that your file was exported using <strong>Ai1 Migration</strong> plugin. ' .
			...
			// Set progress
			Ai1m_Status::info( __( 'Done unpacking archive.', AI1M_PLUGIN_NAME ) );
