...
class Ai1m_Import_Database {

	public static function execute( $params ) {
		global $wpdb;

		// Skip database import
		if ( ! is_file( ai1m_database_path( $params ) ) ) {
			return $params;
		}
		...
		$config = ai1m_read( $handle, filesize( ai1m_package_path( $params ) ) );
		...
		// Set progress
		Ai1m_Status::info( sprintf( __( 'Restoring database...<br />%d%% complete', AI1M_PLUGIN_NAME ), $progress ) );
		...
		foreach ( $blogs as $blog ) {
			...
			foreach ( $home_urls as $home_url ) {
				...
				if ( ! in_array( sprintf( "'%s'", trim( ai1m_blogsdir_path( $blog['Old']['BlogID'] ), '/' ) ), $old_replace_raw_values ) ) {
					$old_replace_raw_values[] = sprintf( "'%s'", trim( ai1m_blogsdir_path( $blog['Old']['BlogID'] ), '/' ) );
					$new_replace_raw_values[] = sprintf( "'%s'", get_option( 'upload_path' ) );
				}

				// Get sites dir Upload Path
				if ( ! in_array( sprintf( "'%s'", trim( ai1m_uploads_path( $blog['Old']['BlogID'] ), '/' ) ), $old_replace_raw_values ) ) {
					$old_replace_raw_values[] = sprintf( "'%s'", trim( ai1m_uploads_path( $blog['Old']['BlogID'] ), '/' ) );
					$new_replace_raw_values[] = sprintf( "'%s'", get_option( 'upload_path' ) );
				}
				...
				if ( defined( 'UPLOADBLOGSDIR' ) ) {
					...
					if ( ! in_array( ai1m_blogsdir_path( $blog['Old']['BlogID'] ), $old_replace_values ) ) {
						$old_replace_values[] = ai1m_blogsdir_path( $blog['Old']['BlogID'] );
						$new_replace_values[] = ai1m_blogsdir_path( $blog['New']['BlogID'] );
					}

					// Get URL encoded Upload Path
					if ( ! in_array( urlencode( ai1m_blogsdir_path( $blog['Old']['BlogID'] ) ), $old_replace_values ) ) {
						$old_replace_values[] = urlencode( ai1m_blogsdir_path( $blog['Old']['BlogID'] ) );
						$new_replace_values[] = urlencode( ai1m_blogsdir_path( $blog['New']['BlogID'] ) );
					}

					// Get URL raw encoded Upload Path
					if ( ! in_array( rawurlencode( ai1m_blogsdir_path( $blog['Old']['BlogID'] ) ), $old_replace_values ) ) {
						$old_replace_values[] = rawurlencode( ai1m_blogsdir_path( $blog['Old']['BlogID'] ) );
						$new_replace_values[] = rawurlencode( ai1m_blogsdir_path( $blog['New']['BlogID'] ) );
					}

					// Get JSON escaped Upload Path
					if ( ! in_array( addcslashes( ai1m_blogsdir_path( $blog['Old']['BlogID'] ), '/' ), $old_replace_values ) ) {
						$old_replace_values[] = addcslashes( ai1m_blogsdir_path( $blog['Old']['BlogID'] ), '/' );
						$new_replace_values[] = addcslashes( ai1m_blogsdir_path( $blog['New']['BlogID'] ), '/' );
					}

					// Get plain Upload Path
					if ( ! in_array( ai1m_uploads_path( $blog['Old']['BlogID'] ), $old_replace_values ) ) {
						$old_replace_values[] = ai1m_uploads_path( $blog['Old']['BlogID'] );
						$new_replace_values[] = ai1m_blogsdir_path( $blog['New']['BlogID'] );
					}

					// Get URL encoded Upload Path
					if ( ! in_array( urlencode( ai1m_uploads_path( $blog['Old']['BlogID'] ) ), $old_replace_values ) ) {
						$old_replace_values[] = urlencode( ai1m_uploads_path( $blog['Old']['BlogID'] ) );
						$new_replace_values[] = urlencode( ai1m_blogsdir_path( $blog['New']['BlogID'] ) );
					}

					// Get URL raw encoded Upload Path
					if ( ! in_array( rawurlencode( ai1m_uploads_path( $blog['Old']['BlogID'] ) ), $old_replace_values ) ) {
						$old_replace_values[] = rawurlencode( ai1m_uploads_path( $blog['Old']['BlogID'] ) );
						$new_replace_values[] = rawurlencode( ai1m_blogsdir_path( $blog['New']['BlogID'] ) );
					}

					// Get JSON escaped Upload Path
					if ( ! in_array( addcslashes( ai1m_uploads_path( $blog['Old']['BlogID'] ), '/' ), $old_replace_values ) ) {
						$old_replace_values[] = addcslashes( ai1m_uploads_path( $blog['Old']['BlogID'] ), '/' );
						$new_replace_values[] = addcslashes( ai1m_blogsdir_path( $blog['New']['BlogID'] ), '/' );
					}
				} else {
					...
					if ( ! in_array( sprintf( '%s/%s/', untrailingslashit( $home_url ), 'files' ), $old_replace_values ) ) {
						...
						$new_replace_values[] = ai1m_uploads_url( $blog['New']['BlogID'] );
					}

					// Get plain Upload Path
					if ( ! in_array( ai1m_blogsdir_path( $blog['Old']['BlogID'] ), $old_replace_values ) ) {
						$old_replace_values[] = ai1m_blogsdir_path( $blog['Old']['BlogID'] );
						$new_replace_values[] = ai1m_uploads_path( $blog['New']['BlogID'] );
					}

					// Get URL encoded Upload Path
					if ( ! in_array( urlencode( ai1m_blogsdir_path( $blog['Old']['BlogID'] ) ), $old_replace_values ) ) {
						$old_replace_values[] = urlencode( ai1m_blogsdir_path( $blog['Old']['BlogID'] ) );
						$new_replace_values[] = urlencode( ai1m_uploads_path( $blog['New']['BlogID'] ) );
					}

					// Get URL raw encoded Upload Path
					if ( ! in_array( rawurlencode( ai1m_blogsdir_path( $blog['Old']['BlogID'] ) ), $old_replace_values ) ) {
						$old_replace_values[] = rawurlencode( ai1m_blogsdir_path( $blog['Old']['BlogID'] ) );
						$new_replace_values[] = rawurlencode( ai1m_uploads_path( $blog['New']['BlogID'] ) );
					}

					// Get JSON escaped Upload Path
					if ( ! in_array( addcslashes( ai1m_blogsdir_path( $blog['Old']['BlogID'] ), '/' ), $old_replace_values ) ) {
						$old_replace_values[] = addcslashes( ai1m_blogsdir_path( $blog['Old']['BlogID'] ), '/' );
						$new_replace_values[] = addcslashes( ai1m_uploads_path( $blog['New']['BlogID'] ), '/' );
					}
				...
				if ( $site_url !== $blog['New']['SiteURL'] ) {
					...
					foreach ( array( $site_url, $site_url_www_inversion ) as $url ) {
						...
						for ( $i = 0; $i < count( $old_schemes ); $i++ ) {
							...
							// Add plain Site URL
							if ( ! in_array( ai1m_url_scheme( $url, $old_schemes[ $i ] ), $old_replace_values ) ) {
								$old_replace_values[] = ai1m_url_scheme( $url, $old_schemes[ $i ] );
								$new_replace_values[] = ai1m_url_scheme( $blog['New']['SiteURL'], $new_schemes[ $i ] );
							}

							// Add URL encoded Site URL
							if ( ! in_array( urlencode( ai1m_url_scheme( $url, $old_schemes[ $i ] ) ), $old_replace_values ) ) {
								$old_replace_values[] = urlencode( ai1m_url_scheme( $url, $old_schemes[ $i ] ) );
								$new_replace_values[] = urlencode( ai1m_url_scheme( $blog['New']['SiteURL'], $new_schemes[ $i ] ) );
							}

							// Add URL raw encoded Site URL
							if ( ! in_array( rawurlencode( ai1m_url_scheme( $url, $old_schemes[ $i ] ) ), $old_replace_values ) ) {
								$old_replace_values[] = rawurlencode( ai1m_url_scheme( $url, $old_schemes[ $i ] ) );
								$new_replace_values[] = rawurlencode( ai1m_url_scheme( $blog['New']['SiteURL'], $new_schemes[ $i ] ) );
							}

							// Add JSON escaped Site URL
							if ( ! in_array( addcslashes( ai1m_url_scheme( $url, $old_schemes[ $i ] ), '/' ), $old_replace_values ) ) {
								$old_replace_values[] = addcslashes( ai1m_url_scheme( $url, $old_schemes[ $i ] ), '/' );
								$new_replace_values[] = addcslashes( ai1m_url_scheme( $blog['New']['SiteURL'], $new_schemes[ $i ] ), '/' );
							}
						}
				...
				if ( $home_url !== $blog['New']['HomeURL'] ) {
					...
					foreach ( array( $home_url, $home_url_www_inversion ) as $url ) {
						...
						for ( $i = 0; $i < count( $old_schemes ); $i++ ) {
							...
							if ( ! in_array( ai1m_url_scheme( $url, $old_schemes[ $i ] ), $old_replace_values ) ) {
								$old_replace_values[] = ai1m_url_scheme( $url, $old_schemes[ $i ] );
								$new_replace_values[] = ai1m_url_scheme( $blog['New']['HomeURL'], $new_schemes[ $i ] );
							}

							// Add URL encoded Home URL
							if ( ! in_array( urlencode( ai1m_url_scheme( $url, $old_schemes[ $i ] ) ), $old_replace_values ) ) {
								$old_replace_values[] = urlencode( ai1m_url_scheme( $url, $old_schemes[ $i ] ) );
								$new_replace_values[] = urlencode( ai1m_url_scheme( $blog['New']['HomeURL'], $new_schemes[ $i ] ) );
							}

							// Add URL raw encoded Home URL
							if ( ! in_array( rawurlencode( ai1m_url_scheme( $url, $old_schemes[ $i ] ) ), $old_replace_values ) ) {
								$old_replace_values[] = rawurlencode( ai1m_url_scheme( $url, $old_schemes[ $i ] ) );
								$new_replace_values[] = rawurlencode( ai1m_url_scheme( $blog['New']['HomeURL'], $new_schemes[ $i ] ) );
							}

							// Add JSON escaped Home URL
							if ( ! in_array( addcslashes( ai1m_url_scheme( $url, $old_schemes[ $i ] ), '/' ), $old_replace_values ) ) {
								$old_replace_values[] = addcslashes( ai1m_url_scheme( $url, $old_schemes[ $i ] ), '/' );
								$new_replace_values[] = addcslashes( ai1m_url_scheme( $blog['New']['HomeURL'], $new_schemes[ $i ] ), '/' );
							}
						}
			...
			if ( $site_url !== site_url() ) {
				...
				foreach ( array( $site_url, $site_url_www_inversion ) as $url ) {
					...
					for ( $i = 0; $i < count( $old_schemes ); $i++ ) {
						...
						if ( ! in_array( ai1m_url_scheme( $url, $old_schemes[ $i ] ), $old_replace_values ) ) {
							$old_replace_values[] = ai1m_url_scheme( $url, $old_schemes[ $i ] );
							$new_replace_values[] = ai1m_url_scheme( site_url(), $new_schemes[ $i ] );
						}

						// Add URL encoded Site URL
						if ( ! in_array( urlencode( ai1m_url_scheme( $url, $old_schemes[ $i ] ) ), $old_replace_values ) ) {
							$old_replace_values[] = urlencode( ai1m_url_scheme( $url, $old_schemes[ $i ] ) );
							$new_replace_values[] = urlencode( ai1m_url_scheme( site_url(), $new_schemes[ $i ] ) );
						}

						// Add URL raw encoded Site URL
						if ( ! in_array( rawurlencode( ai1m_url_scheme( $url, $old_schemes[ $i ] ) ), $old_replace_values ) ) {
							$old_replace_values[] = rawurlencode( ai1m_url_scheme( $url, $old_schemes[ $i ] ) );
							$new_replace_values[] = rawurlencode( ai1m_url_scheme( site_url(), $new_schemes[ $i ] ) );
						}

						// Add JSON escaped Site URL
						if ( ! in_array( addcslashes( ai1m_url_scheme( $url, $old_schemes[ $i ] ), '/' ), $old_replace_values ) ) {
							$old_replace_values[] = addcslashes( ai1m_url_scheme( $url, $old_schemes[ $i ] ), '/' );
							$new_replace_values[] = addcslashes( ai1m_url_scheme( site_url(), $new_schemes[ $i ] ), '/' );
						}
					}
			...
			if ( $home_url !== home_url() ) {
				...
				foreach ( array( $home_url, $home_url_www_inversion ) as $url ) {
					...
					for ( $i = 0; $i < count( $old_schemes ); $i++ ) {
						...
						if ( ! in_array( ai1m_url_scheme( $url, $old_schemes[ $i ] ), $old_replace_values ) ) {
							$old_replace_values[] = ai1m_url_scheme( $url, $old_schemes[ $i ] );
							$new_replace_values[] = ai1m_url_scheme( home_url(), $new_schemes[ $i ] );
						}

						// Add URL encoded Home URL
						if ( ! in_array( urlencode( ai1m_url_scheme( $url, $old_schemes[ $i ] ) ), $old_replace_values ) ) {
							$old_replace_values[] = urlencode( ai1m_url_scheme( $url, $old_schemes[ $i ] ) );
							$new_replace_values[] = urlencode( ai1m_url_scheme( home_url(), $new_schemes[ $i ] ) );
						}

						// Add URL raw encoded Home URL
						if ( ! in_array( rawurlencode( ai1m_url_scheme( $url, $old_schemes[ $i ] ) ), $old_replace_values ) ) {
							$old_replace_values[] = rawurlencode( ai1m_url_scheme( $url, $old_schemes[ $i ] ) );
							$new_replace_values[] = rawurlencode( ai1m_url_scheme( home_url(), $new_schemes[ $i ] ) );
						}

						// Add JSON escaped Home URL
						if ( ! in_array( addcslashes( ai1m_url_scheme( $url, $old_schemes[ $i ] ), '/' ), $old_replace_values ) ) {
							$old_replace_values[] = addcslashes( ai1m_url_scheme( $url, $old_schemes[ $i ] ), '/' );
							$new_replace_values[] = addcslashes( ai1m_url_scheme( home_url(), $new_schemes[ $i ] ), '/' );
						}
					}
		...
		$old_table_prefixes[] = ai1m_servmask_prefix( 'mainsite' );
		$new_table_prefixes[] = ai1m_table_prefix();

		// Set site table prefixes
		foreach ( $blogs as $blog ) {
			if ( ai1m_main_site( $blog['Old']['BlogID'] ) === false ) {
				$old_table_prefixes[] = ai1m_servmask_prefix( $blog['Old']['BlogID'] );
				$new_table_prefixes[] = ai1m_table_prefix( $blog['New']['BlogID'] );
			}
		}

		// Set base table prefixes
		foreach ( $blogs as $blog ) {
			if ( ai1m_main_site( $blog['Old']['BlogID'] ) === true ) {
				$old_table_prefixes[] = ai1m_servmask_prefix( 'basesite' );
				$new_table_prefixes[] = ai1m_table_prefix( $blog['New']['BlogID'] );
			}
		}
		...
		// Set table prefixes
		$old_table_prefixes[] = ai1m_servmask_prefix();
		$new_table_prefixes[] = ai1m_table_prefix();

		// Get database client
		if ( empty( $wpdb->use_mysqli ) ) {
			$mysql = new Ai1m_Database_Mysql( $wpdb );
		} else {
			$mysql = new Ai1m_Database_Mysqli( $wpdb );
		}
		...
		if ( isset( $config['Plugin']['Version'] ) && ( $version = $config['Plugin']['Version'] ) ) {
			if ( $version !== 'develop' && version_compare( $version, '4.10', '<' ) ) {
				$mysql->set_include_table_prefixes( array( ai1m_table_prefix() ) );
				$mysql->flush();
			}
		}

		// Set atomic tables (do not stop the current request for all listed tables if timeout has been exceeded)
		$mysql->set_atomic_tables( array( ai1m_table_prefix() . 'options' ) );

		// Set Visual Composer
		$mysql->set_visual_composer( ai1m_validate_plugin_basename( 'js_composer/js_composer.php' ) );

		// Set BeTheme Responsive
		$mysql->set_betheme_responsive( ai1m_validate_theme_basename( 'betheme/style.css' ) );

		// Import database
		if ( $mysql->import( ai1m_database_path( $params ), $query_offset ) ) {

			// Set progress
			Ai1m_Status::info( __( 'Done restoring database.', AI1M_PLUGIN_NAME ) );
			...
			$total_queries_size = ai1m_database_bytes( $params );
		...
		// Activate plugins
		ai1m_activate_plugins( ai1m_active_servmask_plugins() );
