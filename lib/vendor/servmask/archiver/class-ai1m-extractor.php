...
class Ai1m_Extractor extends Ai1m_Archiver {
	...
	public function get_total_files_count() {
		if ( is_null( $this->total_files_count ) ) {
			...
			if ( @fseek( $this->file_handle, 0, SEEK_SET ) === -1 ) {
				throw new Ai1m_Not_Seekable_Exception( sprintf( 'Unable to seek to beginning of file. File: %s', $this->file_name ) );
			}
			...
			while ( $block = @fread( $this->file_handle, 4377 ) ) {
				...
				if ( ( $data = $this->get_data_from_block( $block ) ) ) {
					...
					if ( @fseek( $this->file_handle, $data['size'], SEEK_CUR ) === -1 ) {
						throw new Ai1m_Not_Seekable_Exception( sprintf( 'Unable to seek to offset of file. File: %s Offset: %d', $this->file_name, $data['size'] ) );
					}
				}
			}
		}
	 ...
	 *
	 * @throws \Ai1m_Not_Directory_Exception
	...
	public function extract_one_file_to( $location, $exclude = array(), $old_paths = array(), $new_paths = array(), &$file_written = 0, &$file_offset = 0 ) {
		if ( false === is_dir( $location ) ) {
			throw new Ai1m_Not_Directory_Exception( sprintf( 'Location is not a directory: %s', $location ) );
		}
		...
		if ( $file_offset > 0 ) {
			if ( @fseek( $this->file_handle, - $file_offset - 4377, SEEK_CUR ) === -1 ) {
				throw new Ai1m_Not_Seekable_Exception( sprintf( 'Unable to seek to offset of file. File: %s Offset: %d', $this->file_name, - $file_offset - 4377 ) );
			}
		}
				...
				if ( ( $data = $this->get_data_from_block( $block ) ) ) {
					...
					if ( $should_exclude_file === false ) {
						...
						if ( @fseek( $this->file_handle, $file_size, SEEK_CUR ) === -1 ) {
							throw new Ai1m_Not_Seekable_Exception( sprintf( 'Unable to seek to offset of file. File: %s Offset: %d', $this->file_name, $file_size ) );
						}
					}
				}
			}
			...
			while ( $file_size > 0 ) {
				...
				if ( $chunk_size > 0 ) {
					...
					if ( ( $file_content = @fread( $this->file_handle, $chunk_size ) ) === false ) {
						throw new Ai1m_Not_Readable_Exception( sprintf( 'Unable to read content from file. File: %s', $this->file_name ) );
					}
					...
					if ( ( $file_bytes = @fwrite( $file_handle, $file_content ) ) !== false ) {
						if ( strlen( $file_content ) !== $file_bytes ) {
							throw new Ai1m_Quota_Exceeded_Exception( sprintf( 'Out of disk space. Unable to write content to file. File: %s', $file_name ) );
						}
					}
