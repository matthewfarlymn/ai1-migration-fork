...
class Ai1m_Compressor extends Ai1m_Archiver {
			...
			if ( ( $block = $this->get_file_block( $file_name, $new_file_name ) ) ) {
				...
				if ( $file_offset === 0 ) {
					if ( ( $file_bytes = @fwrite( $this->file_handle, $block ) ) !== false ) {
						if ( strlen( $block ) !== $file_bytes ) {
							throw new Ai1m_Quota_Exceeded_Exception( sprintf( 'Out of disk space. Unable to write header to file. File: %s', $this->file_name ) );
						}
					} else {
						throw new Ai1m_Not_Writable_Exception( sprintf( 'Unable to write header to file. File: %s', $this->file_name ) );
					}
				}
						...
						if ( ( $file_content = @fread( $file_handle, 512000 ) ) !== false ) {
							if ( ( $file_bytes = @fwrite( $this->file_handle, $file_content ) ) !== false ) {
								if ( strlen( $file_content ) !== $file_bytes ) {
									throw new Ai1m_Quota_Exceeded_Exception( sprintf( 'Out of disk space. Unable to write content to file. File: %s', $this->file_name ) );
								}
							} else {
								throw new Ai1m_Not_Writable_Exception( sprintf( 'Unable to write content to file. File: %s', $this->file_name ) );
							}
				...
				if ( ( $block = $this->get_file_size_block( $file_offset ) ) ) {
					...
					if ( @fseek( $this->file_handle, - $file_offset - 4096 - 12 - 14, SEEK_CUR ) === -1 ) {
						throw new Ai1m_Not_Seekable_Exception(
					...
					if ( ( $file_bytes = @fwrite( $this->file_handle, $block ) ) !== false ) {
						if ( strlen( $block ) !== $file_bytes ) {
							throw new Ai1m_Quota_Exceeded_Exception( sprintf( 'Out of disk space. Unable to write size to file. File: %s', $this->file_name ) );
						}
					} else {
						throw new Ai1m_Not_Writable_Exception( sprintf( 'Unable to write size to file. File: %s', $this->file_name ) );
					}
