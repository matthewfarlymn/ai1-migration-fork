...
if ( class_exists( 'WP_CLI_Command' ) ) {
	class Ai1m_WP_CLI_Command extends WP_CLI_Command {
		/**
		 ...
		 *
		 * Example:
		 * $ wp ai1m backup --replace "wp" "WordPress"
		 * Backup in progress...
		 * Backup complete.
		 * Backup location: /repos/migration/wp/wp-content/ai1m-backups/migration-wp-20170913-095743-931.wpress
		 *
		 * @subcommand backup
		 */
		public function backup( array $args, array $assoc_args ) {
			if ( is_multisite() ) {
				WP_CLI::error( __( 'WPI CLI is not supported in Multisite mode. Please use web interface to create a backup.', AI1M_PLUGIN_NAME ) );
			}

			if ( ! is_dir( AI1WM_STORAGE_PATH ) ) {
				if ( ! mkdir( AI1WM_STORAGE_PATH ) ) {
					WP_CLI::error_multi_line( array(
						sprintf( __( 'Ai1 Migration is not able to create <strong>%s</strong> folder.', AI1M_PLUGIN_NAME ), AI1WM_STORAGE_PATH ),
						__( 'You will need to create this folder and grant it read/write/execute permissions (0777) for the Ai1 Migration plugin to function properly.', AI1M_PLUGIN_NAME ),
					) );
					exit;
				}
			}
			...
			if ( ! is_dir( AI1WM_BACKUPS_PATH ) ) {
				if ( ! mkdir( AI1WM_BACKUPS_PATH ) ) {
					WP_CLI::error_multi_line( array(
						sprintf( __( 'Ai1 Migration is not able to create <strong>%s</strong> folder.', AI1M_PLUGIN_NAME ), AI1WM_BACKUPS_PATH ),
						__( 'You will need to create this folder and grant it read/write/execute permissions (0777) for the Ai1 Migration plugin to function properly.', AI1M_PLUGIN_NAME ),
					) );
					exit;
				}
			...
			if ( isset( $assoc_args['list'] ) ) {
				...
				$backups->setHeaders( array(
					'name' => __( 'Backup name', AI1M_PLUGIN_NAME ),
					'date' => __( 'Date created', AI1M_PLUGIN_NAME ),
					'size' => __( 'Size', AI1M_PLUGIN_NAME ),
				) );

				$model = new Ai1m_Backups;
				foreach ( $model->get_files() as $backup ) {
					$backups->addRow( array(
						'name' => $backup['filename'],
						'date' => sprintf( __( '%s ago', AI1M_PLUGIN_NAME ), human_time_diff( $backup['mtime'] ) ),
			...
			try {
				...
				add_filter( 'ai1m_completed_timeout', '__return_zero' );

				// Remove filters
				remove_filter( 'ai1m_export', 'Ai1m_Export_Clean::execute', 300 );

				// Run filters
				$params = apply_filters( 'ai1m_export', $params );
			...
			WP_CLI::log( __( 'Backup complete.', AI1M_PLUGIN_NAME ) );
			WP_CLI::log( sprintf( __( 'Backup location: %s', AI1M_PLUGIN_NAME ), ai1m_backup_path( $params ) ) );
		}
		 ...
		 *
		 * Example:
		 * $ wp ai1m restore migration-wp-20170913-095743-931.wpress
		 * Restore in progress...
		 * Restore complete.
		 *
		 * @subcommand restore
		...
		public function restore( array $args, array $assoc_args ) {
			if ( is_multisite() ) {
				WP_CLI::error( __( 'WPI CLI is not supported in Multisite mode. Please use web interface to restore a backup.', AI1M_PLUGIN_NAME ) );
			}
			...
			$params = array();
			if ( isset( $args[0] ) && is_file( ai1m_backup_path( array( 'archive' => $args[0] ) ) ) ) {
				$params = array(
					'storage'              => ai1m_storage_folder(),
					'archive'              => $args[0],
					'ai1m_manual_restore' => true,
				);
			} else {
				WP_CLI::error_multi_line( array(
					__( 'A valid backup name must be provided in order to proceed with the restore process.', AI1M_PLUGIN_NAME ),
					__( 'Example: wp ai1m restore migration-wp-20170913-095743-931.wpress', AI1M_PLUGIN_NAME ),
				) );
				exit;
			}
			...
			try {
				...
				// Remove filters
				remove_filter( 'ai1m_import', 'Ai1m_Import_Upload::execute', 5 );
				remove_filter( 'ai1m_import', 'Ai1m_Import_Confirm::execute', 100 );
				remove_filter( 'ai1m_import', 'Ai1m_Import_Clean::execute', 400 );

				// Run filters
				$params = apply_filters( 'ai1m_import', $params );
...
if ( class_exists( 'WP_CLI' ) ) {
	WP_CLI::add_command( 'ai1m', 'Ai1m_WP_CLI_Command', array( 'short_desc' => 'Ai1 Migration Command' ) );
}
