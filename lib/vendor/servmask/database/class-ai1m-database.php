...
abstract class Ai1m_Database {
		...
		if ( is_resource( $this->wpdb->dbh ) ) {
			if ( get_resource_type( $this->wpdb->dbh ) === 'SQL Server Connection' ) {
				throw new Exception(
					...
					'In order to use Ai1 Migration, please change your installation to MySQL and try again. ' .
	...
	public function export( $file_name, &$table_index = 0, &$table_offset = 0 ) {
		// Set file handler
		$file_handler = ai1m_open( $file_name, 'ab' );

		// Write headers
		if ( $table_index === 0 ) {
			ai1m_write( $file_handler, $this->get_header() );
		}
		...
		for ( ; $table_index < count( $tables ); ) {
			...
			if ( $table_offset === 0 ) {
				...
				ai1m_write( $file_handler, $drop_table );
				...
				// Write table statement
				ai1m_write( $file_handler, $create_table );

				// Write end of statement
				ai1m_write( $file_handler, ";\n\n" );
			}
			...
			do {
				...
				if ( $num_rows = $this->num_rows( $result ) ) {
					...
					while ( $row = $this->fetch_assoc( $result ) ) {
						...
						if ( $table_offset % Ai1m_Database::QUERIES_PER_TRANSACTION === 0 ) {
							ai1m_write( $file_handler, "START TRANSACTION;\n" );
						}
						...
						ai1m_write( $file_handler, $table_insert );
						...
						if ( $table_offset % Ai1m_Database::QUERIES_PER_TRANSACTION === 0 ) {
							ai1m_write( $file_handler, "COMMIT;\n" );
						}
					}
				} else {
				...
				// Time elapsed
				if ( ( $timeout = apply_filters( 'ai1m_completed_timeout', 10 ) ) ) {
	...
	public function import( $file_name, &$query_offset = 0 ) {
		...
		// Set file handler
		$file_handler = ai1m_open( $file_name, 'r' );
	...
	protected function replace_table_values_callback( $matches ) {
		...
		$input = Ai1m_Database_Utility::unescape_mysql( $matches[1] );

		// Replace serialized values
		$input = Ai1m_Database_Utility::replace_serialized_values( $this->get_old_replace_values(), $this->get_new_replace_values(), $input );

		// Escape MySQL special characters
		return "'" . Ai1m_Database_Utility::escape_mysql( $input ) . "'";
	}
	...
	protected function replace_visual_composer_values_callback( $matches ) {
		...
		$input = Ai1m_Database_Utility::base64_decode( $matches[1] );

		// Replace values
		$input = Ai1m_Database_Utility::replace_values( $this->get_old_replace_values(), $this->get_new_replace_values(), $input );

		// Encode base64 characters
		return '[vc_raw_html]' . Ai1m_Database_Utility::base64_encode( $input ) . '[/vc_raw_html]';
	}
	...
	protected function replace_betheme_responsive_values_callback( $matches ) {
		...
		// Encode base64 characters
		return "'mfn-page-items','" . Ai1m_Database_Utility::base64_encode( $input ) . "'";
	}
	...
	protected function replace_raw_values( $input ) {
		return Ai1m_Database_Utility::replace_values( $this->get_old_replace_raw_values(), $this->get_new_replace_raw_values(), $input );
	}
