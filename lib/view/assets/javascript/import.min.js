...
jQuery(document).ready(function ($) {
	...
	$('#ai1m-feedback-type-link-1').click(function (e) {
		var radio = $('#ai1m-feedback-type-1');
		if (radio.is(':checked')) {
	...
	// Idea
	$('#ai1m-feedback-type-link-2').click(function (e) {
		var radio = $('#ai1m-feedback-type-2');
		if (radio.is(':checked')) {
			radio.attr('checked', false);
		} else {
			radio.attr('checked', true);
	...
	// Help
	$('#ai1m-feedback-type-3').click(function () {
		// Hide other options
		$('#ai1m-feedback-type-1, #ai1m-feedback-type-2').closest('li').hide();

		// Change placeholder message
		$('.ai1m-feedback-form').find('.ai1m-feedback-message').attr('placeholder', ai1m_locale.how_may_we_help_you);

		// Show feedback form
		$('.ai1m-feedback-form').fadeIn();
	});

	// Cancel feedback form
	$('#ai1m-feedback-cancel').click(function (e) {
		$('.ai1m-feedback-form').fadeOut(function () {
			$('.ai1m-feedback-type').attr('checked', false).closest('li').show();
		});
	...
	// Send feedback form
	$('#ai1m-feedback-submit').click(function (e) {
		var self = $(this);

		var spinner = self.next();
		var type = $('.ai1m-feedback-type:checked').val();
		var email = $('.ai1m-feedback-email').val();
		var message = $('.ai1m-feedback-message').val();
		var terms = $('.ai1m-feedback-terms').is(':checked');

		self.attr('disabled', true);
		spinner.css('visibility', 'visible');

		$.ajax({
			url: ai1m_feedback.ajax.url,
			type: 'POST',
			dataType: 'json',
			async: true,
			data: {
				'secret_key': ai1m_feedback.secret_key,
				'ai1m_type': type,
				'ai1m_email': email,
				'ai1m_message': message,
				'ai1m_terms': +terms
			},
			dataFilter: function dataFilter(data, type) {
				return Ai1m.Util.json(data);
			}
		}).done(function (data) {
			...
			if (data.errors.length > 0) {
				$('.ai1m-feedback .ai1m-message').remove();

				var errorMessage = $('<div />').addClass('ai1m-message ai1m-error-message');
				$.each(data.errors, function (key, value) {
					errorMessage.append($('<p />').text(value));
				});

				$('.ai1m-feedback').prepend(errorMessage);
			} else {
				var successMessage = $('<div />').addClass('ai1m-message ai1m-success-message');
				successMessage.append($('<p />').text(ai1m_locale.thanks_for_submitting_your_feedback));

				$('.ai1m-feedback').html(successMessage);
			}
		});
...
jQuery(document).ready(function ($) {
	...
	$('#ai1m-report-problem-button').click(function (e) {
		$(this).next('.ai1m-report-problem-dialog').toggleClass('ai1m-report-active');

		e.preventDefault();
	});

	$('#ai1m-report-cancel').click(function (e) {
		$(this).closest('.ai1m-report-problem-dialog').removeClass('ai1m-report-active');

		e.preventDefault();
	});

	$('#ai1m-report-submit').click(function (r) {
		var self = $(this);

		var spinner = self.next();
		var email = $('.ai1m-report-email').val();
		var message = $('.ai1m-report-message').val();
		var terms = $('.ai1m-report-terms').is(':checked');

		self.attr('disabled', true);
		spinner.css('visibility', 'visible');

		$.ajax({
			url: ai1m_report.ajax.url,
			type: 'POST',
			dataType: 'json',
			async: true,
			data: {
				'secret_key': ai1m_report.secret_key,
				'ai1m_email': email,
				'ai1m_message': message,
				'ai1m_terms': +terms
			},
			...
			if (data.errors.length > 0) {
				$('.ai1m-report-problem-dialog .ai1m-message').remove();
				...
				$('.ai1m-report-problem-dialog').prepend(errorMessage);
			} else {
				var successMessage = $('<div />').addClass('ai1m-message ai1m-success-message');
				successMessage.append($('<p />').text(ai1m_locale.thanks_for_submitting_your_request));

				$('.ai1m-report-problem-dialog').html(successMessage);

				// Hide message
				setTimeout(function () {
					$('.ai1m-report-problem-dialog').removeClass('ai1m-report-active');
				}, 2000);
			}
		});
...
Import.prototype.setParams = function (params) {
	this.params = Ai1m.Util.list(params);
};
...
Import.prototype.start = function (options, retries) {
	...
	$(window).bind('beforeunload', function () {
		return ai1m_locale.stop_importing_your_website;
	});

	// Set initial status
	this.setStatus({ type: 'info', message: ai1m_locale.preparing_to_import });

	// Set params
	var params = this.params.concat({ name: 'secret_key', value: ai1m_import.secret_key });

	// Set additional params
	if (options) {
		params = params.concat(Ai1m.Util.list(options));
	}

	// Import
	$.ajax({
		url: ai1m_import.ajax.url,
		type: 'POST',
		dataType: 'json',
		data: params,
		dataFilter: function dataFilter(data, type) {
		...
		if (retries >= 5) {
			return self.setStatus({
				type: 'error',
				title: ai1m_locale.unable_to_import,
				message: ai1m_locale.unable_to_start_the_import
			});
		}
...
Import.prototype.confirm = function (options, retries) {
	...
	// Set params
	var params = this.params.concat({ name: 'secret_key', value: ai1m_import.secret_key }).concat({ name: 'priority', value: 150 });
	...
	$.ajax({
		...
		if (retries >= 5) {
			return self.setStatus({
				type: 'error',
				title: ai1m_locale.unable_to_import,
				message: ai1m_locale.unable_to_confirm_the_import
			});
		}
	...
	$.ajax({
		...
		if (retries >= 5) {
			return self.setStatus({
				type: 'error',
				title: ai1m_locale.unable_to_import,
				message: ai1m_locale.unable_to_prepare_blogs_on_import
			});
		}
...
Import.prototype.clean = function (options, retries) {
	...
	// Set initial status
	this.setStatus({ type: 'info', message: ai1m_locale.please_wait_stopping_the_export });

	// Set params
	var params = this.params.concat({ name: 'secret_key', value: ai1m_import.secret_key }).concat({ name: 'priority', value: 400 });
	...
	$.ajax({
		...
		if (retries >= 5) {
			return self.setStatus({
				type: 'error',
				title: ai1m_locale.unable_to_import,
				message: ai1m_locale.unable_to_stop_the_import
			});
		}
...
var Modal = function Modal() {
	...
	this.error = function (params) {
		...
		// Create title
		var title = $('<span></span>').addClass('ai1m-title-red').text(params.title);

		// Create close button
		var closeButton = $('<button type="button" class="ai1m-button-red"></button>').on('click', function () {
			self.destroy();
		});

		// Append text to close button
		closeButton.append(ai1m_locale.close_import);
	...
	this.progress = function (params) {
		if (this.view === 'progress') {
			...
			// Create progress bar
			var progressBar = $('<span class="ai1m-progress-bar"></span>');

			// Create progress bar meter
			this.progress.progressBarMeter = $('<span class="ai1m-progress-bar-meter"></span>').width(params.percent + '%');

			// Create progress bar percent
			this.progress.progressBarPercent = $('<span class="ai1m-progress-bar-percent"></span>').text(params.percent + '%');

			// Create stop import
			var stopButton = $('<button type="button" class="ai1m-button-red"></button>').on('click', function () {
				$(this).attr('disabled', 'disabled');
				self.onStop();
			});

			// Append text to stop button
			stopButton.append('<i class="ai1m-icon-notification"></i> ' + ai1m_locale.stop_import);
	...
	this.confirm = function (params) {
		...
		// Create paragraph to hold mesage
		var message = $('<p class="ai1m-import-modal-content"></p>').html(params.message);

		// Create action section
		var action = $('<div class="ai1m-import-modal-actions"></div>');

		// Create warning
		var warning = $('<i class="ai1m-icon-notification"></i>');

		// Create close button
		var closeButton = $('<button type="button" class="ai1m-button-gray"></button>').on('click', function () {
			$(this).attr('disabled', 'disabled');
			self.onStop();
		});

		// Create confirm button
		var confirmButton = $('<button type="button" class="ai1m-button-green"></button>').on('click', function () {
			$(this).attr('disabled', 'disabled');
			self.onConfirm();
		});
		...
		confirmButton.append(ai1m_locale.confirm_import + ' &gt;');
	...
	this.blogs = function (params) {
		...
		// Create title
		var title = $('<span></span>').addClass('ai1m-title-grey').text(params.title);

		// Create continue button
		var continueButton = $('<button type="button" class="ai1m-button-green"></button>').on('click', function () {
			$(this).attr('disabled', 'disabled');
			self.onBlogs($(this).closest('form').serializeArray());
		});

		// Append text to continue button
		continueButton.append(ai1m_locale.continue_import);
	...
	this.info = function (params) {
		...
		// Create loader
		var loader = $('<span class="ai1m-loader"></span>');

		// Create warning
		var warning = $('<p></p>').html(ai1m_locale.please_do_not_close_this_browser);

		// Create notice to be displayed during import process
		var notice = $('<div class="ai1m-import-modal-notice"></div>');
	...
	this.done = function (params) {
		...
		// Create title
		var title = $('<span></span>').addClass('ai1m-title-green').text(params.title);
	...
	// Create the overlay
	this.overlay = $('<div class="ai1m-overlay"></div>');

	// Create the modal container
	this.modal = $('<div class="ai1m-modal-container"></div>');
...
jQuery(document).ready(function ($) {
  ...
  $('.ai1m-expandable > div.ai1m-button-main').on('click', function () {
    $(this).parent().toggleClass('ai1m-open');
  });
});

global.Ai1m = jQuery.extend({}, global.Ai1m, { FileUploader: FileUploader, Feedback: Feedback, Report: Report, Import: Import });
...
FileUploader.prototype.init = function () {
	...
	var formElement = $('#ai1m-import-form');
	var selectElement = $('#ai1m-import-file');
	var dropElement = $('#ai1m-drag-drop-area');
	...
	dropElement.on('dragenter', function (e) {
		dropElement.addClass('ai1m-drag-over');
		e.preventDefault();
	});
	...
	dropElement.on('dragleave', function (e) {
		dropElement.removeClass('ai1m-drag-over');
		e.preventDefault();
	});
...
FileUploader.prototype.upload = function (file, retries) {
	...
	var chunkSize = file.size > ai1m_uploader.chunk_size ? ai1m_uploader.chunk_size : file.size;
	var formData = this.getFormData(file, chunkSize);

	fetch(ai1m_uploader.url, {
		method: 'POST',
		credentials: 'include',
		body: formData
		...
		switch (response.status) {
			case 413:
				ai1m_uploader.chunk_size = chunkSize / 2;
				return _this2.upload(file);
			case 200:
				return response.text().then(function (text) {
					var extractedJsonText = Ai1m.Util.json(text);
		...
		var timeout = retries * 1000;
		if (retries >= 30) {
			return _this2.onError(new Error(ai1m_locale.problem_while_uploading_your_file));
		}
...
FileUploader.prototype.getFormData = function (file, chunkSize) {
	var params = ai1m_uploader.params;
...
FileUploader.prototype.checkSize = function (file) {
	var size = parseInt(ai1m_uploader.filters.ai1m_archive_size, 10);

	if (file.size > size && size !== 0) {
		throw new Error(ai1m_locale.invalid_archive_size);
	}
};

FileUploader.prototype.checkExtension = function (file) {
	var extensions = ai1m_uploader.filters.ai1m_archive_extension;
	...
	if (!matchExtension) {
		throw new Error(ai1m_locale.invalid_archive_extension);
	}
};
...
FileUploader.prototype.onBeforeUpload = function (file) {
	...
	var storage = Ai1m.Util.random(12);
	var options = Ai1m.Util.form('#ai1m-import-form').concat({ name: 'storage', value: storage }).concat({ name: 'archive', value: file.name });

	// Set global params
	this.model.setParams(options);

	// Set multipart params
	$.extend(ai1m_uploader.params, {
		storage: storage,
		archive: file.name
	});
...
FileUploader.prototype.onError = function (error) {
	this.model.setStatus({ type: 'error', title: ai1m_locale.unable_to_import, message: error.message });
};
